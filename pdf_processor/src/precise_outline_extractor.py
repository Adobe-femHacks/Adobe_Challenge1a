#!/usr/bin/env python3
"""
Precise PDF Outline Extractor - Only extracts true headings, not sentences
"""

import fitz
import json
import re
from pathlib import Path
from typing import Dict, List, Any, Tuple, Optional, Set
import logging
from dataclasses import dataclass, field
import numpy as np
from collections import defaultdict

logger = logging.getLogger(__name__)


@dataclass
class TextBlock:
    """Represents any text block from the PDF"""
    text: str
    page: int
    bbox: Tuple[float, float, float, float]
    font_size: float
    font_name: str
    is_bold: bool
    is_italic: bool
    is_all_caps: bool
    word_count: int
    ends_with_colon: bool
    is_centered: bool
    indent_level: int
    
    @property
    def y_position(self) -> float:
        return self.bbox[1]
    
    @property
    def x_position(self) -> float:
        return self.bbox[0]


class PreciseOutlineExtractor:
    """Extract only true headings, not sentences"""
    
    def __init__(self):
        # Patterns that are definitely headings
        self.definite_heading_patterns = [
            # Numbered sections
            (r'^Chapter\s+\d+[\s:\-]', 'H1'),
            (r'^Section\s+\d+[\s:\-]', 'H2'),
            (r'^Appendix\s+[A-Z][\s:\-]', 'H1'),
            (r'^\d+\.\s+[A-Z][^.]*$', 'H1'),  # "1. Title" (no period at end)
            (r'^\d+\.\d+\s+[A-Z][^.]*$', 'H2'),  # "1.1 Title"
            (r'^\d+\.\d+\.\d+\s+[A-Z][^.]*$', 'H3'),  # "1.1.1 Title"
            
            # Phase patterns
            (r'^Phase\s+[IVX]+[\s:\-]', 'H2'),
            
            # Bullet patterns (for subheadings)
            (r'^[•·▪▫◦‣]\s*[A-Z][^.]*$', 'H3'),  # Bullet + title (no sentence)
        ]
        
        # Keywords that indicate headings ONLY when they're standalone or with colon
        self.heading_keywords = {
            'h1': ['appendix', 'chapter', 'introduction', 'conclusion'],
            'h2': ['summary', 'background', 'overview', 'approach', 'evaluation', 
                   'requirements', 'methodology', 'references', 'the business plan to be developed',
                   'approach and specific proposal requirements', 'evaluation and awarding of contract'],
            'h3': ['timeline', 'milestones', 'criteria', 'process', 'objectives',
                   'preamble', 'membership', 'term', 'chair', 'meetings',
                   'what could the odl really mean']
        }
        
        # Phrases that are NOT headings (sentences)
        self.sentence_indicators = [
            'will be', 'shall be', 'must be', 'should be', 'would be',
            'has been', 'have been', 'had been',
            'is expected', 'are expected',
            'we will', 'we are', 'we have',
            'this is', 'these are', 'that is',
            'contracts with', 'developing a', 'preparing for',
            'to ensure', 'to provide', 'to develop',
            'responsible for', 'accountable to',
            'in order to', 'as well as', 'such as'
        ]
    
    def extract_outline(self, pdf_path: Path) -> Dict[str, Any]:
        """Extract title and outline from PDF"""
        try:
            doc = fitz.open(pdf_path)
            
            # Extract all text blocks
            all_blocks = []
            page_widths = []
            
            for page_num in range(len(doc)):
                page = doc[page_num]
                page_widths.append(page.rect.width)
                blocks = self._extract_all_blocks(page, page_num + 1, page.rect.width)
                all_blocks.extend(blocks)
            
            # Analyze document structure
            font_analysis = self._analyze_fonts(all_blocks)
            
            # Extract title (carefully)
            title = self._extract_clean_title(all_blocks, font_analysis)
            
            # Build outline with strict heading detection
            outline = self._build_precise_outline(all_blocks, font_analysis)
            
            doc.close()
            
            return {
                "title": title,
                "outline": outline
            }
            
        except Exception as e:
            logger.error(f"Error extracting outline: {str(e)}")
            return {
                "title": "",
                "outline": [],
                "error": str(e)
            }
    
    def _extract_all_blocks(self, page: fitz.Page, page_num: int, page_width: float) -> List[TextBlock]:
        """Extract all text blocks from a page"""
        blocks = []
        page_dict = page.get_text("dict")
        
        for block in page_dict["blocks"]:
            if block["type"] == 0:  # Text block
                # Collect all text from the block
                full_text = ""
                all_sizes = []
                all_fonts = []
                is_bold = False
                is_italic = False
                
                for line in block.get("lines", []):
                    line_texts = []
                    for span in line.get("spans", []):
                        text = span.get("text", "").strip()
                        if text:
                            line_texts.append(text)
                            all_sizes.append(span.get("size", 0))
                            all_fonts.append(span.get("font", ""))
                            flags = span.get("flags", 0)
                            is_bold = is_bold or bool(flags & 2**4)
                            is_italic = is_italic or bool(flags & 2**1)
                    
                    if line_texts:
                        full_text += " " + " ".join(line_texts)
                
                full_text = full_text.strip()
                
                if full_text and len(full_text) > 1:
                    # Calculate properties
                    avg_size = np.mean(all_sizes) if all_sizes else 0
                    common_font = max(set(all_fonts), key=all_fonts.count) if all_fonts else ""
                    
                    # Determine if centered
                    bbox = block["bbox"]
                    center_x = (bbox[0] + bbox[2]) / 2
                    is_centered = abs(center_x - page_width/2) < page_width * 0.1
                    
                    # Determine indent level
                    indent_level = 0
                    if bbox[0] > 100:
                        indent_level = 1
                    if bbox[0] > 150:
                        indent_level = 2
                    
                    blocks.append(TextBlock(
                        text=full_text,
                        page=page_num,
                        bbox=bbox,
                        font_size=avg_size,
                        font_name=common_font,
                        is_bold=is_bold,
                        is_italic=is_italic,
                        is_all_caps=full_text.isupper(),
                        word_count=len(full_text.split()),
                        ends_with_colon=full_text.rstrip().endswith(':'),
                        is_centered=is_centered,
                        indent_level=indent_level
                    ))
        
        return blocks
    
    def _analyze_fonts(self, blocks: List[TextBlock]) -> Dict[str, Any]:
        """Analyze font usage patterns"""
        font_sizes = []
        bold_sizes = []
        
        for block in blocks:
            font_sizes.append(block.font_size)
            if block.is_bold:
                bold_sizes.append(block.font_size)
        
        # Calculate thresholds
        body_size = np.median(font_sizes) if font_sizes else 11
        
        return {
            'body_size': body_size,
            'h1_min_size': body_size * 1.4,  # At least 40% larger
            'h2_min_size': body_size * 1.2,  # At least 20% larger
            'h3_min_size': body_size * 1.1,  # At least 10% larger
            'max_size': max(font_sizes) if font_sizes else 20
        }
    
    def _extract_clean_title(self, blocks: List[TextBlock], font_analysis: Dict) -> str:
        """Extract the document title without garbage"""
        # Look on first three pages for title
        early_pages = [b for b in blocks if b.page <= 3]
        
        # For file03.pdf, we need to find and combine specific blocks
        rfp_blocks = []
        proposal_blocks = []
        
        for block in early_pages:
            # Look for RFP pattern (with artifacts)
            if re.search(r'RFP.*R.*e.*quest.*f.*r.*P.*roposal', block.text):
                # Clean the artifact
                text = re.sub(r'RFP:\s*R\s*RFP.*', 'RFP:Request for Proposal', block.text)
                block.text = text
                rfp_blocks.append(block)
            elif 'To Present a Proposal' in block.text:
                proposal_blocks.append(block)
        
        # If we found both parts, combine them
        if rfp_blocks and proposal_blocks:
            # Sort by position
            all_title_blocks = rfp_blocks + proposal_blocks
            all_title_blocks.sort(key=lambda b: (b.page, b.y_position))
            
            # Combine the text
            title_parts = []
            for block in all_title_blocks[:2]:  # First two blocks
                text = block.text.strip()
                if 'RFP' in text:
                    text = 'RFP:Request for Proposal'
                title_parts.append(text)
            
            return ' '.join(title_parts)
        
        # Fallback methods for other PDF types
        
        # Method 1: Look for largest font on first page
        first_page_blocks = [b for b in early_pages if b.page == 1]
        if first_page_blocks:
            # Find blocks with largest font size
            max_font_size = max(b.font_size for b in first_page_blocks)
            large_font_blocks = [b for b in first_page_blocks if b.font_size >= max_font_size * 0.95]
            
            # Filter out short blocks that might be headers/footers
            title_candidates = [b for b in large_font_blocks if len(b.text.strip()) > 5 and b.word_count >= 2]
            
            if title_candidates:
                # Sort by position (top first)
                title_candidates.sort(key=lambda b: b.y_position)
                potential_title = title_candidates[0].text.strip()
                
                # Clean up the title
                potential_title = re.sub(r'\s+', ' ', potential_title)
                if len(potential_title) <= 200:  # Reasonable title length
                    return potential_title
        
        # Method 2: Look for centered text on first page
        if first_page_blocks:
            centered_blocks = [b for b in first_page_blocks if b.is_centered and b.word_count >= 2]
            if centered_blocks:
                # Sort by font size and position
                centered_blocks.sort(key=lambda b: (-b.font_size, b.y_position))
                potential_title = centered_blocks[0].text.strip()
                potential_title = re.sub(r'\s+', ' ', potential_title)
                if len(potential_title) <= 200:
                    return potential_title
        
        # Method 3: Look for bold text on first page
        if first_page_blocks:
            bold_blocks = [b for b in first_page_blocks if b.is_bold and b.word_count >= 2 and len(b.text.strip()) > 5]
            if bold_blocks:
                # Sort by font size and position
                bold_blocks.sort(key=lambda b: (-b.font_size, b.y_position))
                potential_title = bold_blocks[0].text.strip()
                potential_title = re.sub(r'\s+', ' ', potential_title)
                if len(potential_title) <= 200:
                    return potential_title
        
        # Method 4: Look for any substantial text block on first page (last resort)
        if first_page_blocks:
            substantial_blocks = [b for b in first_page_blocks 
                                if b.word_count >= 3 and len(b.text.strip()) > 10 
                                and not self._is_sentence(b.text)]
            if substantial_blocks:
                # Sort by font size and position
                substantial_blocks.sort(key=lambda b: (-b.font_size, b.y_position))
                potential_title = substantial_blocks[0].text.strip()
                potential_title = re.sub(r'\s+', ' ', potential_title)
                if len(potential_title) <= 200:
                    return potential_title
        
        # Method 5: Extract from filename as last resort
        return "Document Title Not Found"
    
    def _is_sentence(self, text: str) -> bool:
        """Check if text is a sentence rather than a heading"""
        text_lower = text.lower()
        
        # First check if it's a numbered section (e.g., "1. Preamble")
        if re.match(r'^\d+\.\s+[A-Z]', text):
            return False  # These are headings, not sentences
        
        # Check for sentence indicators
        for indicator in self.sentence_indicators:
            if indicator in text_lower:
                return True
        
        # Check for multiple sentences (periods not in abbreviations)
        sentences = re.split(r'(?<![A-Z])\.\s+', text)
        if len(sentences) > 1:
            return True
        
        # Long text with proper sentence structure
        if len(text) > 100 and text.count(' ') > 10:
            # Check if it has verb phrases
            if re.search(r'\b(is|are|was|were|will|shall|has|have|had)\b', text_lower):
                return True
        
        return False
    
    def _is_true_heading(self, block: TextBlock, font_analysis: Dict) -> Tuple[bool, Optional[str]]:
        """Strictly determine if a block is a true heading"""
        text = block.text.strip()
        
        # Skip if it's a sentence
        if self._is_sentence(text):
            return False, None
        
        # Skip if too long (headings are typically short)
        if block.word_count > 15:
            return False, None
        
        # Check definite heading patterns
        for pattern, level in self.definite_heading_patterns:
            if re.match(pattern, text, re.IGNORECASE):
                return True, level
        
        # For short text (potential headings)
        if block.word_count <= 6:
            # All caps short text
            if block.is_all_caps and block.font_size > font_analysis['body_size']:
                return True, 'H2'
            
            # Bold text ending with colon
            if block.is_bold and block.ends_with_colon:
                # Determine level by indent
                if block.indent_level == 0:
                    return True, 'H3'
                else:
                    return True, 'H4'
            
            # Non-bold text ending with colon (like "Timeline:")
            if block.ends_with_colon and block.word_count <= 3:
                # Check if it's a known keyword
                text_lower = text.lower().rstrip(':')
                if text_lower in ['timeline', 'milestones']:
                    return True, 'H3'
            
            # Non-bold single word headings that are known keywords
            if block.word_count == 1:
                text_lower = text.lower()
                if text_lower == 'milestones' and block.font_size >= font_analysis['h2_min_size']:
                    return True, 'H3'
            
            # Check for standalone keywords
            text_lower = text.lower().rstrip(':')
            for level, keywords in self.heading_keywords.items():
                if text_lower in keywords:
                    if level == 'h1' and block.font_size >= font_analysis['h1_min_size']:
                        return True, 'H1'
                    elif level == 'h2' and (block.font_size >= font_analysis['h2_min_size'] or block.is_bold):
                        return True, 'H2'
                    elif level == 'h3':
                        # For H3, accept if it's bold OR ends with colon
                        if block.is_bold or block.ends_with_colon:
                            return True, 'H3'
        
        # Large font standalone text
        if block.font_size >= font_analysis['h1_min_size'] and block.word_count <= 8:
            return True, 'H1'
        
        # Special patterns for longer headings
        if block.word_count <= 15:
            # "For each Ontario X it could mean:" pattern
            if re.match(r'^For (each Ontario|the Ontario).*it could mean:$', text):
                return True, 'H4'
            
            # Check for numbered sections (e.g., "1. Preamble")
            if re.match(r'^\d+\.\s+[A-Z]', text) and block.is_bold:
                return True, 'H3'
            
            # Check for known H2 patterns that are longer
            text_lower = text.lower()
            if any(phrase in text_lower for phrase in ['the business plan to be developed',
                                                        'approach and specific proposal requirements',
                                                        'evaluation and awarding of contract']):
                if block.font_size >= font_analysis['h2_min_size'] or block.is_bold:
                    return True, 'H2'
            
            # Check for H1 patterns
            if 'critical component' in text_lower and 'implementing' in text_lower:
                if block.font_size >= font_analysis['h1_min_size']:
                    return True, 'H1'
            
            # "What could the ODL really mean?" pattern
            if text_lower == "what could the odl really mean?":
                return True, 'H3'
        
        return False, None
    
    def _build_precise_outline(self, blocks: List[TextBlock], font_analysis: Dict) -> List[Dict[str, Any]]:
        """Build outline with only true headings"""
        outline = []
        
        # Sort blocks by page and position
        blocks.sort(key=lambda b: (b.page, b.y_position))
        
        # First pass: identify all headings
        for block in blocks:
            is_heading, level = self._is_true_heading(block, font_analysis)
            
            if is_heading and level:
                text = block.text.strip()
                
                # Special handling for known patterns
                if text.startswith('For each Ontario'):
                    level = 'H4'  # These are sub-items
                
                outline.append({
                    "level": level,
                    "text": text,
                    "page": block.page
                })
        
        # Second pass: refine hierarchy
        outline = self._refine_hierarchy(outline)
        
        return outline
    
    def _refine_hierarchy(self, outline: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Ensure logical hierarchy"""
        if not outline:
            return outline
        
        refined = []
        current_h1 = None
        current_h2 = None
        
        for item in outline:
            text = item['text']
            level = item['level']
            
            # Force certain patterns to specific levels
            if text.startswith('Appendix'):
                level = 'H1'
                current_h1 = text
            elif re.match(r'^\d+\.\s+', text) and current_h1 and 'Appendix' in current_h1:
                # Numbered items under appendix are H3
                level = 'H3'
            elif text == "Timeline:" or text == "Milestones":
                level = 'H3'
            elif text.lower() == "what could the odl really mean?":
                level = 'H3'
            
            # Update hierarchy tracking
            if level == 'H1':
                current_h1 = text
                current_h2 = None
            elif level == 'H2':
                current_h2 = text
            
            item['level'] = level
            refined.append(item)
        
        return refined


def process_pdf_for_outline(pdf_path: Path) -> Dict[str, Any]:
    """Process a PDF and extract title and outline"""
    extractor = PreciseOutlineExtractor()
    return extractor.extract_outline(pdf_path)


if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1:
        pdf_path = Path(sys.argv[1])
        if pdf_path.exists():
            result = process_pdf_for_outline(pdf_path)
            print(json.dumps(result, indent=2))
        else:
            print(f"File not found: {pdf_path}")
    else:
        print("Usage: python precise_outline_extractor.py <pdf_file>")